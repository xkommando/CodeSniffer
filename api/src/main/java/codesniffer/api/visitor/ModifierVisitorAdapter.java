...
public abstract class ModifierVisitorAdapter<A> implements GenericVisitor<Node, A> {
	...
	@Override public Node visit(final AnnotationDeclaration n, final A arg) {
		if (n.getDocComment() != null) {
			n.setDocComment((DocumentComment) n.getDocComment().accept(this, arg));
		}
	...
	@Override public Node visit(final DocumentComment n, final A arg) {
		return n;
	}
	...
	@Override public Node visit(MultiTypeParameter n, A arg) {
    	visit((BaseParameter) n, arg);
        List<Type> tls = n.getTypes();
    	List<Type> types = new ArrayList<>(tls);
    	for (Type type : tls) {
