...
package codesniffer.api.visitor;
...
public final class DumpVisitor implements VoidVisitor<Object> {
    ...
    private void printJavadoc(final DocumentComment javadoc, final Object arg) {
        if (javadoc != null) {
    ...
    public void visit(final ClassOrInterfaceDeclaration n, final Object arg) {
        printJavaComment(n.getComment(), arg);
        printJavadoc(n.getDocComment(), arg);
    ...
    @Override
    public void visit(final DocumentComment n, final Object arg) {
        printer.print("/**");
    ...
    public void visit(final TryStmt n, final Object arg) {
        printJavaComment(n.getComment(), arg);
        printer.print("try ");
        List<VariableDeclarationExpr> res = n.getResources();
    ...
    private void printOrphanCommentsBeforeThisChildNode(final Node node) {
        ...
        List<Node> kids = parent.getChildrenNodes();
        if (kids == null || kids.size() == 0)
            return;

        List<Node> everything = new ArrayList<>(kids);
...
//    }
    ...
    private void printOrphanCommentsEnding(final Node node) {
        List<Node> kids = node.getChildrenNodes();
        if (kids == null || kids.size() == 0)
            return;
